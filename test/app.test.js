...
test('AGENT.md 가 쓰기 규칙과 경계를 담는다', () => {
  ...
  // ⚠️ **추천 금지에서 원금 기준으로 바뀌었다.** 안 하는 이유가 규제인 줄 알았는데
  //    실은 이해관계였고, 돈동생은 중개 수수료가 없어서 그 이유가 없다. 대신
  //    원금이 줄 수 있는 쪽(금융투자상품)은 남긴다 — 이해관계가 없는 것과
  //    원금 손실 책임은 다른 문제다. 두 층이 다 살아 있어야 경계가 성립한다.
  assert.match(A.AGENT_GUIDE, /최선을 답하세요/, '원금이 안 주는 층에서 결론까지 내라고 해야 한다');
  assert.match(A.AGENT_GUIDE, /비중을 처방하거나 사고팔라고 하지 마세요/,
    '원금이 줄 수 있는 층의 경계가 빠졌다');
  assert.match(A.AGENT_GUIDE, /예금자보호/, '원금 손실 가능 상품은 보호 여부를 밝혀야 한다');
  // 금리는 우리가 검증하지 않는 유일한 숫자다. 출처가 섞이면 틀린 금리 하나가
  // 검증된 계산까지 같이 죽인다.
  assert.match(A.AGENT_GUIDE, /돈동생이 검증하지 않았습니다/,
    '검색해온 숫자와 계산한 숫자의 구분이 빠졌다');
...
test('AGENT.md 가 쓰기 전에 물어보라고 한다', () => {
  // ⚠️ **한때 정반대를 적었다** — "허락을 매번 묻지 마세요". 물으면 사용자가
  //    답을 안 하고 대화가 끝나서 파일이 0개가 된다는 이유였는데, 실측에서
  //    그 문장이 포함된 파일 전체가 **프롬프트 인젝션으로 분류**됐다.
  //
  //    남의 드라이브에 확인 없이 쓰지 않는 건 **지켜야 할 동작**이다.
  //    데이터 파일로 그걸 이기려 든 게 잘못이었다. 대신 **제안**하게 한다 —
  //    사용자는 이 폴더가 있다는 걸 잊고 있고, 물어보지 않으면 기억할
  //    기회 자체가 안 생긴다.
  const A = loadApp();
  assert.match(A.AGENT_GUIDE, /확인 없이 쓰지 마세요/, '확인 절차를 빼고 있다');
  assert.match(A.AGENT_GUIDE, /먼저 제안해 주세요/, '제안조차 안 하면 기회가 없다');
  assert.doesNotMatch(A.AGENT_GUIDE, /묻지 마세요/,
    '확인 절차를 무력화하는 문장이 다시 들어왔다');
});

test('AGENT.md 가 언제 제안할지를 말하고, 브레이크가 액셀을 덮지 않는다', () => {
  // ⚠️ 처음엔 '쓰지 마세요' 가 14문장, '쓰세요' 가 8문장이었다. 자제 규칙을
  //    쌓다 보니 **안내문 전체가 순 감속**이 됐고, 실제로 AI 는 아무것도
  //    안 적었다. 트리거가 명시돼야 하고, 감속이 가속보다 많으면 안 된다.
  const A = loadApp();
  assert.match(A.AGENT_GUIDE, /언제 물어보나/, '제안할 시점을 알려주는 절이 없다');
  assert.match(A.AGENT_GUIDE, /안 물어보면 그냥 사라집니다/, '왜 지금인지가 빠졌다');
  ...
  assert.ok(
    A.AGENT_GUIDE.indexOf('### 언제 물어보나') < A.AGENT_GUIDE.indexOf('### 안 써도 되는 때'),
    '자제 규칙이 제안하라는 절보다 위에 있다');
});
