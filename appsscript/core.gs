 ...
 *
 * 여기서 판단은 하지 않는다. "이 이체가 문제다" 는 LLM 이 말하고,
 * 우리는 "이체를 본인/타인으로 갈랐더니 타인 쪽 순액이 얼마" 를 만든다.
 *
 * ━━ 이 파일의 존재 이유 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *
 * 원본 거래 수천 건을 LLM 에 던지면 이체의 비대칭을 못 찾는다.
...
KM.analyze = (function () {
   ...
   *   수입 − 지출 + **남과 오간 이체 순액**
   *
   * 이체를 통째로 빼면 한 해 흐름이 −1,000만원대로 나오는데, 순자산이
   * +1,000만원대이고 부채가 0이면 앞뒤가 안 맞는다.
   * 통째로 더하면 연동 안 된 내 계좌의 돈을 새 돈으로 센다.
 ...
 *
 * 3. **투자성 자산 금액은 실수다** (소수점 아래가 열 자리 넘게 붙어 온다).
 *    → 원 단위로 반올림.
 *
 ...
 * 데이터를 못 믿거나 없는 사실을 지어낸다. 실측에서 카테고리 24만원대,
 * 계좌 4만원대, 이체 상대 305만원대가 말없이 사라지고 있었다.
 *
 * ━━ 커넥터 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *
...
KM.aggregate = (function () {
  ...
  function build(extract, opts) {
    ...
    //
    //    이런 식이다: 카카오뱅크 세이프박스로 매달 100만원을 옮기면 적요가
    //    상품명이라 본인 계좌로 안 잡힌다. 실제 여유가 월 150만이어도
    //    pace.monthly 는 50만으로 나온다. 3배 차이다.
  ...
  /**
   * 양끝 달은 대개 잘려 있다. 실측에서 마지막 달이 **8일치 65만원대**인데
   ...
   *    merchants 합이 지출을 만원 남짓 넘었다. 빼기로 정의하면 구조적으로 안 깨진다.
   */
  ...
  function recurringBlock(txns) {
    ...
    //    합이 안 맞는데 소비자는 그 사실을 모른다. 샘플에서도 월 7만원대가
    //    설명 없이 사라져 있었다.
   ...
   *    실측에서 13만원대가 조용히 증발했다. **그 분리가 이 블록의 값어치다** —
