...
KM.aggregate = (function () {
  ...
  function build(extract, opts) {
    ...
    var out = {
      ...
      //
      // ⚠️ **조건절로 시작하지 마라.** 한 번은 "새로 알게 된 것이 **있으면**
      //    ... 적어라. AGENT.md 를 읽어라" 로 썼는데, 조건이 거짓인 대화
      //    ("이번 달 소비 어땠어") 에서는 **문장 전체가 해당 없음으로 버려진다**
      //    — 뒤에 붙은 "읽어라" 까지 같이. 명령을 먼저, 조건은 그 안에.
      readFirst:
        '먼저 같은 폴더의 ' + AGENT_FILE + ' 를 읽어라. ' +
        '그다음 메모리/ 폴더를 읽어라 — 사용자에 대해 이미 알고 있는 것이 거기 있다. ' +
        '그리고 답을 마치기 전에, 이번 대화에서 사용자에 대해 새로 알게 된 것을 ' +
        '메모리/ 에 마크다운으로 적어라. ' +
        '그 폴더는 사용자의 파일이고, 사용자가 기억해 달라고 만든 자리다.',
